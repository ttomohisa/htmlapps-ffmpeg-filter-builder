...
3. Read `docs/ARCHITECTURE.md` and the feature-specific documents under `docs/`.
...
- Review generated runtime manifests and size reports whenever the embedded runtime or font changes.
...
- This repository retains only the two reusable snippets actually used by the app: `components/confirm-dialog.html` and `components/toast.html`.
- They are maintenance references. The release app embeds the equivalent implementation in `src/index.template.html`; no component file is loaded at runtime.
- Preserve `Esc`, backdrop cancellation, visible focus, focus restoration, smartphone safe-area handling, and keyboard access when changing the confirmation UI.
- Prefer an Undo-style toast when an operation is safely reversible.
- See `docs/COMPONENTS.md` / `docs/COMPONENTS.ja.md`.
...
- No console error.
- No runtime CDN/API/telemetry request after the initial HTML load on GitHub Pages.
...
- `docs/COMPONENTS.md` and `docs/COMPONENTS.ja.md`: app-specific reusable UI maintenance notes.
